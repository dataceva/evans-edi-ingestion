	...
	public void generateMercuryGateXML(Shipment shipment) throws Exception {
		for (IEDIMessageBody<ShipmentBody> body : shipment.getBody()) {
			try {
				...
				if(null != trailer.getTotalWeightAndChargeSummary() && null != trailer.getTotalWeightAndChargeSummary().getCharge()) {
					charge.setAmount(BigDecimal.valueOf(trailer.getTotalWeightAndChargeSummary().getCharge() / 100));
					charge.setRate(BigDecimal.valueOf(trailer.getTotalWeightAndChargeSummary().getCharge() / 100));
				}else {
				...
				charge.setWeight(BigDecimal.valueOf(0.0));
